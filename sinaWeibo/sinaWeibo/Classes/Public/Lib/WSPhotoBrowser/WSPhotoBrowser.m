...
@interface WSPhotoBrowser() <UIScrollViewDelegate>
@property (nonatomic,strong) UIScrollView *scrollView;
@property (nonatomic,assign) BOOL hasShowedPhotoBrowser;
@property (nonatomic,strong) UILabel *indexLabel;
@property (nonatomic,strong) UIActivityIndicatorView *indicatorView;
@property (nonatomic,strong) UIButton *showToolbarBtn;
@end
...
- (void)viewDidLoad
{
    ...
    [WSNotificationCenter addObserver:self selector:@selector(saveImage) name:WSBrowserToolBarViewSaveImageButtonDidClickNotification object:nil];
    
    [WSNotificationCenter addObserver:self selector:@selector(retweetStatus:) name:WSBrowserToolBarViewRetweetButtonDidClickNotification object:nil];
    
    [WSNotificationCenter addObserver:self selector:@selector(composeAttitude:) name:WSBrowserToolBarViewAttitudeButtonDidClickNotification object:nil];
}

- (void)retweetStatus:(NSNotification *)noti {
...
// 赞
- (void)composeAttitude:(NSNotification *)noti {
...
- (void)setUpFrames
{
    ...
    //设置所有WSPhotoBrowserView的frame
    [_scrollView.subviews enumerateObjectsUsingBlock:^(WSPhotoBrowserView *obj, NSUInteger idx, BOOL *stop) {
    ...
    CGFloat saveBtnW = 55;
    CGFloat saveBtnH = 30;
    CGFloat saveBtnX = kAPPWidth - _showToolbarBtn.width;
    CGFloat saveBtnY = _indexLabel.y;
    _showToolbarBtn.frame = CGRectMake(saveBtnX,saveBtnY, saveBtnW, saveBtnH);
}
...
- (void)addScrollView
{
    ...
    for (int i = 0; i < self.imageCount; i++) {
        WSPhotoBrowserView *view = [[WSPhotoBrowserView alloc] init];
...
//    indexLabel.clipsToBounds = YES;
    ...
    // 2.保存图片按钮
    UIButton *showToolbarBtn = [[UIButton alloc] init];
    [showToolbarBtn setImage:[UIImage imageNamed:@"navigationbar_more"] forState:UIControlStateNormal];
    [showToolbarBtn setImage:[UIImage imageNamed:@"navigationbar_more_highlighted"] forState:UIControlStateHighlighted];
    [showToolbarBtn addTarget:self action:@selector(showToolBarView) forControlEvents:UIControlEventTouchUpInside];
    _showToolbarBtn = showToolbarBtn;
    [self.view addSubview:showToolbarBtn];
}

- (void)showToolBarView {
    WSBrowserToolBarView *toolBar = [[WSBrowserToolBarView alloc] init];
...
- (void)saveImage
{
    ...
    WSPhotoBrowserView *currentView = _scrollView.subviews[index];
...
{
    WSPhotoBrowserView *view = (WSPhotoBrowserView *)recognizer.view;
...
{
    WSPhotoBrowserView *view = _scrollView.subviews[index];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    for (WSPhotoBrowserView *view in _scrollView.subviews) {
